# INLA model of WNV season end (last day of transmission) analysis — gridMET 1980-2024
...
last_mean <- mean(wnv_data$LastValid)
last_sd   <- sd(wnv_data$LastValid)
wnv_data  <- wnv_data %>%
  mutate(outcome_scaled = as.numeric(scale(LastValid)))
...
##--- El Nino binary models (is_elnino: 1 if seasonal ONI >= 0.5)
...
total_change <- function(model, quant = "mean") {
  ...
  b1 <- cf["year_c",      quant] * last_sd
  b2 <- if ("I(year_c^2)" %in% rownames(cf)) cf["I(year_c^2)", quant] * last_sd else 0
  b1 * (t2 - t1) + b2 * (t2^2 - t1^2)
}

bt <- function(model, label) {
  (model$summary.fixed[, c("mean", "0.025quant", "0.975quant")] * last_sd) %>%
    as.data.frame() %>%
...
model_list <- list(
  ...
  "M23: linear year x county slope + El Nino binary"        = model_m23,
  "M24: quadratic year + county slope + El Nino binary"     = model_m24,
  "M25: linear year x elevation + El Nino binary"           = model_m25,
  "M26: quadratic year x elevation + El Nino binary"        = model_m26,
  "M27: linear year x latitude + El Nino binary"            = model_m27,
  "M28: quadratic year x latitude + El Nino binary"         = model_m28
)
...
model_comparison
write.csv(model_comparison, "results/tables/gridMET_last_day_model_comparison.csv", row.names = FALSE)

###----- Fixed effects (back-transformed to days)

fixed_effects <- bind_rows(lapply(names(model_list), function(nm) bt(model_list[[nm]], nm)))
fixed_effects
write.csv(fixed_effects, "data/gridMET_last_day_fixed_effects.csv", row.names = FALSE)

###----- Save models

saveRDS(model_m1,  "data/gridMET_last_day_m1_linear.rds")
saveRDS(model_m2,  "data/gridMET_last_day_m2_quad.rds")
saveRDS(model_m3,  "data/gridMET_last_day_m3_linear_enso.rds")
saveRDS(model_m4,  "data/gridMET_last_day_m4_quad_enso_rw2.rds")
saveRDS(model_m5,  "data/gridMET_last_day_m5_slope.rds")
saveRDS(model_m6,  "data/gridMET_last_day_m6_quad_slope.rds")
saveRDS(model_m7,  "data/gridMET_last_day_m7_slope_enso.rds")
saveRDS(model_m8,  "data/gridMET_last_day_m8_quad_slope_enso_rw2.rds")
saveRDS(model_m9,  "data/gridMET_last_day_m9_elev_linear.rds")
saveRDS(model_m10, "data/gridMET_last_day_m10_elev_quad.rds")
saveRDS(model_m11, "data/gridMET_last_day_m11_elev_linear_enso.rds")
saveRDS(model_m12, "data/gridMET_last_day_m12_elev_quad_enso_rw2.rds")
saveRDS(model_m13, "data/gridMET_last_day_m13_lat_linear.rds")
saveRDS(model_m14, "data/gridMET_last_day_m14_lat_quad.rds")
saveRDS(model_m15, "data/gridMET_last_day_m15_lat_linear_enso.rds")
saveRDS(model_m16, "data/gridMET_last_day_m16_lat_quad_enso_rw2.rds")
saveRDS(model_m17, "data/gridMET_last_day_m17_linear_enso_rw2.rds")
saveRDS(model_m18, "data/gridMET_last_day_m18_slope_linear_enso_rw2.rds")
saveRDS(model_m19, "data/gridMET_last_day_m19_elev_linear_enso_rw2.rds")
saveRDS(model_m20, "data/gridMET_last_day_m20_lat_linear_enso_rw2.rds")
saveRDS(model_m21, "data/gridMET_last_day_m21_linear_elnino.rds")
saveRDS(model_m22, "data/gridMET_last_day_m22_quad_elnino.rds")
saveRDS(model_m23, "data/gridMET_last_day_m23_slope_linear_elnino.rds")
saveRDS(model_m24, "data/gridMET_last_day_m24_slope_quad_elnino.rds")
saveRDS(model_m25, "data/gridMET_last_day_m25_elev_linear_elnino.rds")
saveRDS(model_m26, "data/gridMET_last_day_m26_elev_quad_elnino.rds")
saveRDS(model_m27, "data/gridMET_last_day_m27_lat_linear_elnino.rds")
saveRDS(model_m28, "data/gridMET_last_day_m28_lat_quad_elnino.rds")
